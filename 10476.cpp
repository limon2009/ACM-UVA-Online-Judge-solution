	...
	while(j<Sp) {
		...
		if(!strcmp(Text,"ENDMESSAGE")) j++;
			...
			for(i = 0; i<l-2; i++){
				...
				F_SP[p]++;
				if(p>Max_sp) Max_sp = p;
				if(p<min_sp) min_sp = p;
			}
		}
	}
	...
	while(j<Non) {
		...
		if(!strcmp(Text,"ENDMESSAGE")) j++;
			...
			for(i = 0; i<l-2; i++){
				p = Val(Text[i]-32,Text[i+1]-32,Text[i+2]-32);
				F_NON[p]++;
				if(p>Max_non) Max_non = p;
				if(p<min_non) min_non = p;
			}
		}
	}
}
...
void ReadMs() {
	...
	while(j<Ms) {
		...
		if(!strcmp(Text,"ENDMESSAGE")){
			...
			Similarity_1();
			Similarity_2();
			if(Sm1>Sm2) printf("spam\n");
			else printf("non-spam\n");
			for(i = min_ms; i<= Max_ms; i++) F_MS[i] = 0;
		}
		else {
			l = strlen(Text);
			if(l <3) continue;
			...
			for(i = 0; i<l-2; i++){
				p = Val(Text[i]-32,Text[i+1]-32,Text[i+2]-32);
				F_MS[p]++;
				if(p>Max_ms) Max_ms = p;
				if(p<min_ms) min_ms = p;
			}
		}
	}
