...
int Pro(int k) {
	...
	l2 = strlen(s2);
	l3 = strlen(num);
	if(k != l1 + l2 + l3 + 2) return 0;
	if(l3 >4) return 0;
	for(i = 0; i<l1; i++){
		if(isupper(s1[i]) || !isalpha(s1[i])) return 0;
		F1[s1[i] - 'a'] ++;
	}
	for(i = 0; i<l2; i++){
		if(isupper(s2[i]) || !isalpha(s2[i])) return 0;
		F2[s2[i] - 'a'] ++;
	}
	if(l1 == 0 || l1 > 1000 || l2>2000 || l2 == 0) return 0;
	for(i = 0; num[i]; i++)
...
//	freopen("h.txt","r",stdin);
	while(gets(input)) {
		...
		d = Cal(l);
		if(d) printf(" The solution is accepted\n");
