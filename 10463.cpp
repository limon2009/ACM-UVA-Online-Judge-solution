...
int IDS(int move, int t, int cu,int limit ) {
   ...
   r = move/C;
   c = move%C;
   if(move == t && limit == cu) return 1;
   if(limit == cu) return 0;
   if(F[limit][move] == 1) return 0;
   if(B[move].colour == 1) return 0;
   B[move].colour = 1;
   F[limit][move] = 1;
   for(i = 0; i<8; i++){
		nr = r+X[i];
		...
		if(nr>=R || nr<0 || nc>=C || nc<0) continue;
		ss = nr*C+nc;
		if(IDS(ss,t,cu+1,limit)) return 1;
...
void Cal() {
	...
	if(!P[s] && s) { printf("The knight takes %d prime moves.\n",s); return;}
	if(s == 0){ printf("Destination is not reachable.\n");  return;}
	for(i = 0; i<I; i++) {
		if(pp[i]>(R*C)/2) break;
		m = 0;
		for(j = 0; j<R; j++){
