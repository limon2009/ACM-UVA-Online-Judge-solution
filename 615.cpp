...
void Com() {
    ...
    int i;
    if( K != N+1) { printf("Case %d is not a tree.\n",Kase++); return; }
    if(Root()) { printf("Case %d is not a tree.\n",Kase++); return; }
    for(i = 0; i<K; i ++) {
       V[i].cl = 'W';

    }
   DFS(0);
   if(Un()) { printf("Case %d is not a tree.\n",Kase++); return; }
...
int main()  {
	...
	while(1)  {
	   ...
	   if(g == 1) { printf("Case %d is not a tree.\n",Kase++);continue; }
	
	   if(l == 0 || g == 1) { printf("Case %d is a tree.\n",Kase++);continue; }
		MAKELINK(k,N);
	    Com();
	    for(i = 0; i<32; i ++)
