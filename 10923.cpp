...
ss Pos(int r, int c, int x, int y) {
	...
	for(i = -1; i<2; i++) {
		for(int j = -1; j<2; j++) {
			...
			nx = r + i;
			ny = c + j;
			if(nx >= 9 || nx < 0 || ny >=8 || ny< 0) continue;
			d = Dis(nx, ny, x, y);
			if( d < min) {
				t.r = nx;
				t.c = ny;
...
int Set(int r, int c, ss t) {
	...
	for(i = 0; i<V.size(); i++) {
		...
		if(k.B[V[i].r][V[i].c] == 'S') return 0;
		
		else if(k.B[V[i].r][V[i].c] == '#') {
			k.enemy --;
			if(k.enemy == 0) return 1;
		}
...
/*	if(h == 0) {
	...
	else printf("I'm the king of the Seven Seas!\n");
}
