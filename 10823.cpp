...
int Pos(string ob, int x, int y, int ind) {
	...
	if(ob == CR) {
		...
		if(y == sy && (x>=rx && x<=sx)) return 1;
		if(x == rx && ( y>= ry && y<=py)) return 1;
		if(y == py && (x>=px && x<=qx)) return 1;
		if(x == sx && ( y>= sy && y<= qy)) return 1;
		if(x<rx || x>sx) return 2;
		if(y<sy || y>qy) return 2;
	}
	return 0;
}
...
int main() {
	...
	while(ks--) {
		...
		for(int i = 0; i<Ob.size(); i++) Len[i].clear();
		Ob.clear();
	}
	return 0;
