...
void ReadCase() {
	...
	while(1) {
		...
		if(!a) break;
		u = M[a];
		if(!u) { u = tn++; Num[tn-1] = a; M[a] = tn-1; }
		scanf("%d%d%d",&b,&c,&d);
		v = M[d];
		if(!v) { v = tn++; Num[tn-1] = d; M[d] = tn-1; }
...
void Cal() {
	...
	while(1) {
		...
		else printf("AT %.4d CALL TO %.4d RINGS %.4d\n",t,n,v);
		for(t = 1; t<=tn; t++) Fg[t] = 0;
	}
}
void Free() {
	int i;
	for(i = 0; i<10000; i++) M[i] = 0;
	for(i = 0; i<=tn; i++) Call[i].ind = 0;
}
