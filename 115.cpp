...
int isParent(int n, int level, int v) {
	int i, k;
	if(n == v) return level;
...
int isChild(int n, int level, int v) {
	int i, k;
	if(n == 0) return 0;
	if(n == v) return level;
...
int isSibling(int u, int v) {
	return Node[u].parent == Node[v].parent;
}

void Cousin1(int n, int level) {
	if(n == 0) return;
	Fg[n] = level;
