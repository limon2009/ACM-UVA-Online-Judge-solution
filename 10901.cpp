...
int lind, rind, ftime, fside;
int Cross[maxn], T, N, M;
...
void Move() {
	...
	if(lind == Left.size()) {
		...
		for(i = rind; i<Right.size(); i++) {
			if(k == N || Right[i].time > ftime) break;
	...
	if(rind == Right.size()) {
		...
		for(i = lind; i<Left.size(); i++) {
			if(k == N || Left[i].time > ftime) break;
	...
	if(Left[lind].time < Right[rind].time) {
		...
		if(fside == 0)
			...
			k = 0;
			for(i = rind; i<Right.size(); i++) {
				if(Right[i].time > ftime || k == N) break;
				Cross[Right[i].no] = ftime + T;
				k++;
			}
			rind = i;
	...
	if(Left[lind].time > Right[rind].time) {
		...
		if(fside == 0){
			k = 0;
			for(i = lind; i<Left.size(); i++) {
				if(Left[i].time > ftime || k == N) break;
				Cross[Left[i].no] = ftime + T;
				k++;
			}
			lind = i;
