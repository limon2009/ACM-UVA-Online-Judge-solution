...
int FW(int r, int c, char p) {
	...
	if( p == 'W' && c == N-1)  return 1;
	if(p == 'B' && r == N-1) return 1;
	Temp[r][c] = 'X';
	for(int i = 0; i<4; i++) {
		int nr = r + MR[i];
		int nc = c + MC[i];
		if(FW(nr,nc,p) ==1) return 1;
	}
	return 0; 
}
...
int Win(char p, int n) {
	int i, j;
	if(p == 'W' && whi > n) return 1;
	if(p == 'B' && bla > n) return 1;
	if(p == 'W') {
		j = 0;
		for(i = 0; i<N; i++)
			if(B[i][0] == 'W') j++;
...
int F(int r, int c, char p, int level) {
	...
	if(Temp[r][c] == 'U') ff = 0;
	if( p == 'W' && c == N-1 && Win(p,level))  return 1;
	if(p == 'B' && r == N-1 && Win(p,level)) return 1;
	Temp[r][c] = 'X';
	for(int i = 0; i<4; i++) {
		int nr = r + MR[i];
		int nc = c + MC[i];
		if(F(nr,nc,p, level+1) ==1) return 1;
	   if(Temp[nr][nc] == 'U') ff = 1; 
	}
	return 0; 
}
