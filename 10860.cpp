...
int N;
char SS[10002];
char HavS[128], HavX[128];
char Temp[102];
char Fg[10002];
int R[10002], Len, MinLen, ff;
...
int Match(int ind, int st) {
	int i, k;
	k = Len - st + 1;
	if(XX[ind].len > k) return 0;
	for(i = 0; i<XX[ind].len; i++) {
		if(XX[ind].str[i] != SS[st+i]) return 0;
	}
	return 1;
}
void Recur(int n, int level) {
	int i;
	if(Fg[n] && R[n] <= level) return;
...
void Cal() {
	...
	for(i = 0; i<N+N; i++) {
		if(Match(i,0)) Recur(XX[i].len-1,1);
	}
	if(ff) printf("%d.\n",MinLen);
