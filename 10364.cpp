...
int OK(int k) {
   if(V[k-1].flag == 0) return 1;
   if(V[k-1].flag == 1 && (V[k].val == V[k-1].val))return 0;
   return 1;
}
...
int Recur(int index, int level, int sum, int av, int c, int st) {
	...
	if(sum == av){ 
		...
		for(i = 1; i<M; i++) 
			if(V[i].flag == 1) break;
		if(Recur(i,level+1,sum,av,c,1)== 1) return 1;
	}
	
	V[index].flag  = 0;
    if(level == M-1) return 0;
    for(i = st; i<M; i++) {
		if(V[i].flag == 1 && OK(i)){
		   if(Recur(i,level+1,sum,av,c,i+1) == 1) return 1;
		}
	}
	V[index].flag = 1;
	return 0;
