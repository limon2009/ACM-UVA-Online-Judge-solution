...
void Recur(int x, int y, int p, int q, int sizE, int v, int level) {
		...
		A[level] = v;
		o = AllOne(x,y,p,q);
		if(o == sizE*sizE) { Get(level); return; }
		if(o == 0) return;
...
void Print() {
	...
	while(k < tn) {
		printf("%d",Seq[k]);
		c = 1;
		for(i = k+1; i<tn && c<12; i++,c++) printf(" %d",Seq[i]);
		k = i;
		printf("\n");
	}
}
...
void Fill(int x, int y, int p, int q, int sizE, int level) {
		...
		n = A[level];
		switch(n) {
		case 1:	Fill(x, y,   x+j-1,  y+j-1, j, level+1);	break;
		case 2: Fill(x, y+j, x+j-1, q,   j, level+1);		break;
		case 3: Fill(x+j, y, p, y+j-1, j,  level+1);		break;
		case 4:	Fill(x+j, y+j, p, q, j, level+1);			break;
		}
}
void Base5(int n) {
	INDEX = 0;
