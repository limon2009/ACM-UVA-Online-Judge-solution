...
void GetOperand(char dummy[], int len) {
	...
	while(j<len) {
		for(i = j; i<len && isdigit(dummy[i]); i++) xx[k++] = dummy[i];
		xx[k] = NULL;
		Eq[op++].opd = atoi(xx);
		k = 0;
...
void GetOperator(char dummy[], int len) {
	...
	for(i = 1; i<len; i++) {
		if(!isdigit(dummy[i])) {
			if(isdigit(dummy[i-1])) Eq[k++].opt = dummy[i];
			else {
				if(dummy[i] == '-') Eq[k].opd *= -1;
			}
		}
	}
}
...
int DivMult() {
	...
	for(i = 0; i+1<op; i++) {
		if(Eq[i].opt =='*' || Eq[i].opt == '/') {
			...
			else Eq[i].opd = Eq[i].opd / Eq[i+1].opd;
			for(j = i+1; j+1<op; j++)
				Eq[j].opd = Eq[j+1].opd;
			for(j = i; j+2<op; j++) Eq[j].opt = Eq[j+1].opt;
			op--;
			Print();
			return 1;
		}
