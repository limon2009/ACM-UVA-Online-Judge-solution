...
bool operator <(const ss &p, const ss &q) {
	if(p.A.compare(q.A) != 0) return true;
	if(p.r != q.r || p.c != q.c) return true;
	return false;
}
...
void BFS(int r, int c, string st) {
	...
	while(!Q.empty()) {
		...
		Q.pop();
		Ext(t.M);
		if( GoLeft(t.r,t.c,t.Path,t.M, t.level)) {return;}
		if(GoRight(t.r,t.c,t.Path,t.M, t.level)) {return;}
		if(   GoUp(t.r,t.c,t.Path,t.M, t.level)) {return;}
		if( GoDown(t.r,t.c,t.Path,t.M, t.level)) {return;}
	}
}
