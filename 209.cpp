...
int isTri() {
	...
	if(pp[0].line == pp[1].line) {
		dis = P[1] - P[0];
		if((pp[2].line - pp[1].line) != dis) return 0;
		if(pp[0].left != pp[2].left || pp[2].right != pp[1].right) return 0;
	}
	else {
		if(pp[1].line != pp[2].line) return 0;
		dis = P[2] - P[1];
		if((pp[1].line - pp[0].line) != dis) return 0;
		if(pp[0].right != pp[1].right || pp[0].left != pp[2].left) return 0;
	}
	return 1;
}
...
int isPara() {
	int dis;
	if(pp[0].line != pp[1].line) return 0;
	if(pp[2].line != pp[3].line) return 0;
	dis = P[1] - P[0];
	if((P[3] - P[2])!= dis || (pp[2].line - pp[0].line) != dis) return 0;
	if((pp[3].line - pp[1].line) != dis) return 0;
	if(pp[0].left == pp[2].left && pp[1].left == pp[3].left) return 1;
	if(pp[0].right == pp[2].right && pp[1].right == pp[3].right) return 1;
	return 0;
}
...
int isPara2() {
	...
	if((pp[1].line - pp[0].line) != dis) return 0;
	if(pp[0].right != pp[1].right || pp[0].left != pp[2].left) return 0;
	if(pp[1].left != pp[3].left || pp[2].right != pp[3].right) return 0;
	return 1;
}
